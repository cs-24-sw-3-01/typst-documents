...
It is important to clarify that the fourth element, "Technology", might be mistaken for the same as the final part of the previously mentioned PACT framework in section 4, since both are referred to as "Techonlogy". However, the techonology in the PACT framework focuses on input, output, communication (between hardware and software), and the content. On the oterh hand, for the Factor criterion, technology is defined as "both the techonolgy used to develop the system and the technology on which the system will run"#todo[p. 40 - DD] , which provides a more general formulation. 
